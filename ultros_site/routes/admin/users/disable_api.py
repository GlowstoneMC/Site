...
class DisableUserAPIRoute(BaseSink):
    route = re.compile("/admin/users/disable_api/(?P<user_id>.*)")
    ...
    def __call__(self, req, resp, user_id):
        ...
        try:
            ...
            if db_user.api_enabled:
                db_user.api_enabled = False

                return self.render_template(
                    req, resp, "admin/message_gate.html",
                    gate_message=Message(
                        "success", "API access revoked", "You have revoked API access for {}".format(db_user.username)
                    ),
                    redirect_uri="/admin/users"
                )
            else:
                return self.render_template(
                    req, resp, "admin/message_gate.html",
                    gate_message=Message(
                        "warning", "MFA already disabled", "That user does not have API access."
                    ),
                    redirect_uri="/admin/users"
                )
